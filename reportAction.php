...
$query = $slave_db->query("SELECT attendance_logs.*,IF(is_process=1,'Upload Done','Pending') as uploadStatus,DATE_FORMAT(attendance_date, '%d-%m-%Y %h:%i %p') as createdTitle FROM attendance_logs WHERE is_process = 1 AND (date(attendance_date) >= '$fromDt' and date(attendance_date)  <= '$toDt')");
//echo "<pre>";
//print_r($query);
//exit;
// final data
