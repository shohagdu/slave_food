...
// curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
    ...
    if($response['status'] == 'success'){
		...
		$updatePrimaryID =  array_column($responseData, 'log_id');
		$impID=implode(",",$updatePrimaryID);
        $query = $slave_db->query("UPDATE attendance_logs SET is_process=1 WHERE id IN($impID)");
    }
